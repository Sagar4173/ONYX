import {
  SparklesIcon, ExclamationTriangleIcon, FireIcon,
  LightBulbIcon, CheckCircleIcon, CodeBracketIcon as CodeIcon,
  DocumentTextIcon, ChartBarIcon, BoltIcon, ClockIcon
    ...
    <div className="space-y-6">
      {aiLoading ? (
        <div className="glass-container rounded-xl p-8 text-center">
          <svg className="h-8 w-8 text-blue-400 animate-spin mx-auto mb-4" fill="none" viewBox="0 0 24 24">
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="relative inline-flex items-center justify-center">
                <svg className="w-24 h-24 transform -rotate-90">
                  ...
                  <circle cx="48" cy="48" r="40" stroke="currentColor" strokeWidth="8" fill="transparent"
                    ...
                    strokeDasharray={`${(aiAnalysis.security_score || 0) * 2.51} 251`} strokeLinecap="round"
                  />
                </svg>
            ...
            <div className="glass-container rounded-xl p-6 text-center">
              ...
              <div className={`inline-flex items-center justify-center w-24 h-24 rounded-full ${
                aiAnalysis.risk_level === "CRITICAL" ? "bg-red-500/20 border-2 border-red-500" :
                aiAnalysis.risk_level === "HIGH" ? "bg-orange-500/20 border-2 border-orange-500" :
                aiAnalysis.risk_level === "MEDIUM" ? "bg-yellow-500/20 border-2 border-yellow-500" :
                ...
                <span className={`text-lg font-bold ${
                  ...
                  "text-green-400"
                }`}>{aiAnalysis.risk_level || "N/A"}</span>
              </div>
              <p className="text-xs text-gray-500 mt-2">Risk Score: {aiAnalysis.risk_score || "N/A"}/100</p>
            </div>
            ...
            <div className="glass-container rounded-xl p-6 text-center">
              ...
              <p className="text-sm font-medium text-white mt-3">{aiAnalysis.estimated_fix_time || "N/A"}</p>
            </div>
          </div>
          ...
          {aiAnalysis.executive_summary && (
            <div className="glass-container rounded-xl p-6">
              ...
              <p className="text-gray-300 leading-relaxed whitespace-pre-line">{aiAnalysis.executive_summary}</p>
            </div>
          )}
              ...
              <ul className="space-y-3">
                {aiAnalysis.priority_findings.map((finding, index) => (
                  <li key={index} className="flex items-start">
                    <span className="flex-shrink-0 w-6 h-6 bg-red-500 text-white text-xs rounded-full flex items-center justify-center mr-3 mt-0.5">{index + 1}</span>
          ...
          {aiAnalysis.secure_code_examples && Object.keys(aiAnalysis.secure_code_examples).length > 0 && (
            <div className="glass-container rounded-xl p-6">
              <h3 className="text-lg font-semibold text-white mb-4 flex items-center">
              ...
              <div className="space-y-4">
                {Object.entries(aiAnalysis.secure_code_examples).map(([key, example], index) => (
                  <div key={index} className="bg-gray-900/50 rounded-lg p-4">
                    ...
                    <pre className="text-sm text-green-300 overflow-x-auto"><code>{example}</code></pre>
                  </div>
                ))}
              </div>
            </div>
                  ...
                  {aiAnalysis.compliance_impact.overall_impact && (
                    <div className="flex items-center space-x-2">
                      ...
                      <span className={`px-2 py-1 rounded text-sm font-medium ${
                        aiAnalysis.compliance_impact.overall_impact.toLowerCase().includes("high") ? "bg-red-500/20 text-red-400" :
                        aiAnalysis.compliance_impact.overall_impact.toLowerCase().includes("medium") ? "bg-yellow-500/20 text-yellow-400" :
                        "bg-green-500/20 text-green-400"
                      }`}>{aiAnalysis.compliance_impact.overall_impact}</span>
                    </div>
                  )}
                  {aiAnalysis.compliance_impact.frameworks_affected && (
                    <div><span className="text-gray-400 font-medium">Frameworks Affected:</span><p className="text-gray-300 mt-1">{aiAnalysis.compliance_impact.frameworks_affected}</p></div>
                  )}
                  {aiAnalysis.compliance_impact.analysis && (
                    <div><span className="text-gray-400 font-medium">Analysis:</span><p className="text-gray-300 mt-1 leading-relaxed">{aiAnalysis.compliance_impact.analysis}</p></div>
                  )}
                  {aiAnalysis.compliance_impact.required_actions && (
                    <div><span className="text-gray-400 font-medium">Required Actions:</span><p className="text-gray-300 mt-1">{aiAnalysis.compliance_impact.required_actions}</p></div>
                  )}
                </div>
              )}
            </div>
            ...
            <div className="glass-container rounded-xl p-6">
              ...
              <div className="space-y-3">
                {aiAnalysis.attack_vectors.map((vector, index) => (
                  <div key={index} className="flex items-start bg-red-900/20 border border-red-500/30 rounded-lg p-3">
                ...
                {aiAnalysis.remediation_roadmap.map((phase, index) => (
                  <div key={index} className="border border-gray-700 rounded-lg overflow-hidden">
                    <div className={`p-4 flex items-center justify-between ${
                      phase.priority === "CRITICAL" ? "bg-red-900/30" :
                      phase.priority === "HIGH" ? "bg-orange-900/30" :
                      phase.priority === "MEDIUM" ? "bg-yellow-900/30" :
                      "bg-green-900/30"
                    }`}>
                      <div className="flex items-center space-x-3">
                        <span className={`px-2 py-1 rounded text-xs font-bold ${
                          phase.priority === "CRITICAL" ? "bg-red-500 text-white" :
                          phase.priority === "HIGH" ? "bg-orange-500 text-white" :
                          phase.priority === "MEDIUM" ? "bg-yellow-500 text-black" :
          ...
          {aiAnalysis.model_used && (
            <div className="glass-container rounded-xl p-4 bg-gray-800/30">
              <p className="text-xs text-gray-500 text-center">
                🤖 Analysis generated by <span className="text-blue-400">{aiAnalysis.model_used}</span>
                {aiAnalysis.generated_at && ` on ${new Date(aiAnalysis.generated_at).toLocaleString()}`}
              </p>
            </div>
          )}
        </>
        ...
        <div className="glass-container rounded-xl p-8 text-center">
          ...
          <p className="text-gray-400 mb-4">
            {aiError ? "Failed to load AI analysis. Please try again later." : "AI analysis has not been generated for this report yet."}
          </p>
          <p className="text-sm text-gray-500">
            AI analysis is automatically generated when a scan completes. If you're seeing this message, the analysis may still be processing or may not have been triggered.
          </p>
        </div>
      )}
    </div>
